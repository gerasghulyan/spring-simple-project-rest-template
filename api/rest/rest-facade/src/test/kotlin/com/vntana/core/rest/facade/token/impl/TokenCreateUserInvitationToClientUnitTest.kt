...
class TokenCreateUserInvitationToClientUnitTest : AbstractTokenFacadeUnitTest() {
    ...
    fun `test when precondition failed`() {
        ...
        expect(preconditionChecker.checkCreateTokenUserInvitationToClient(request))
                .andReturn(SingleErrorWithStatus.of(406, TokenErrorResponseModel.WRONG_REQUEST))
        replayAll()
        assertBasicErrorResultResponse(tokenServiceFacade.createTokenInvitationUserToClient(request), TokenErrorResponseModel.WRONG_REQUEST)
        verifyAll()
    }
