 ...
 * Time: 5:07 PM
 */
class UserRolePreconditionCheckerCheckRevokeClientsRolesByUserAndOrganizationComponentUnitTest : AbstractUserRoleFacadePreconditionCheckerComponentUnitTest() {
    ...
    fun `test when organization not found`() {
        ...
        replayAll()
        preconditionChecker.checkRevokeClientsRolesByUserAndOrganization(request).let {
    ...
    fun `test when user has organization related role`() {
        ...
        replayAll()
        preconditionChecker.checkRevokeClientsRolesByUserAndOrganization(request).let {
            assertThat(it.error).isEqualTo(UserRoleErrorResponseModel.REQUESTED_USER_HAS_ORGANIZATION_ROLE)
    ...
    fun `test when user does not have a client related role in organization`() {
        ...
        replayAll()
        preconditionChecker.checkRevokeClientsRolesByUserAndOrganization(request).let {
            assertThat(it.error).isEqualTo(UserRoleErrorResponseModel.USER_NOT_FOUND_IN_ORGANIZATION_WITH_CLIENT_ROLE)
            assertThat(it.httpStatus).isEqualTo(HttpStatus.SC_NOT_FOUND)
        }
        verifyAll()
    }
    ...
    fun test() {
        ...
        expect(userService.existsByUuid(request.userUuid)).andReturn(true)
        expect(userRoleService.findByOrganizationAndUser(request.organizationUuid, request.userUuid)).andReturn(Optional.empty())
        expect(userRoleService.findAllClientOrganizationRoleByOrganizationAndUser(request.organizationUuid, request.userUuid)).andReturn(listOf(commonTestHelper.buildUserClientAdminRole()))
        replayAll()
        assertThat(preconditionChecker.checkRevokeClientsRolesByUserAndOrganization(request).isPresent).isEqualTo(false)
        verifyAll()
    }
}