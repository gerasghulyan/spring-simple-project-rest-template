...
class InvitationUserToClientAcceptWebTest : AbstractInvitationUserWebTest() {
    ...
    fun `test when already has any role`() {
        ...
        val invitedUserUuid = userResourceTestHelper.persistUser(userResourceTestHelper.buildCreateUserRequest(email = invitationEmail)).response().uuid
        userRoleResourceTestHelper.grantUserClientRole(invitedUserUuid, clientUuid = clientUuid, userRole = UserRoleModel.CLIENT_ORGANIZATION_ADMIN)
        ...
        assertBasicErrorResultResponse(HttpStatus.CONFLICT, invitationUserResourceClient.acceptInvitationForClient(request), InvitationUserErrorResponseModel.USER_ALREADY_HAS_ROLE_IN_CLIENT)
    }
