...
class UserRoleRevokeUserOrganizationClientsRolesWebTest : AbstractUserRoleWebTest() {
    ...
    fun `test when user has organization role`() {
        ...
        assertBasicErrorResultResponse(HttpStatus.CONFLICT, result, UserRoleErrorResponseModel.REQUESTED_USER_HAS_ORGANIZATION_ROLE)
    }

    @Test
    fun `test when user does not have client role in organization`() {
        val userResponse = userResourceTestHelper.persistUser().response()
        val userResponse2 = userResourceTestHelper.persistUser().response()
        ...
        userRoleResourceTestHelper.grantUserClientRole(userUuid = userResponse2.uuid, clientUuid = clientUuid, userRole = UserRoleModel.CLIENT_ORGANIZATION_CONTENT_MANAGER)
        userRoleResourceTestHelper.revokeClientRole(userUuid = userResponse2.uuid, clientUuid = clientUuid, userRole = UserRoleModel.CLIENT_ORGANIZATION_CONTENT_MANAGER)
        val result = userRoleResourceClient.revokeUserOrganizationClientsRoles(userRoleResourceTestHelper.buildUserRoleRevokeOrganizationClientsRequest(
                organizationUuid = userResponse.organizationUuid,
                userUuid = userResponse2.uuid
        ))
        assertBasicErrorResultResponse(HttpStatus.NOT_FOUND, result, UserRoleErrorResponseModel.USER_NOT_FOUND_IN_ORGANIZATION_WITH_CLIENT_ROLE)
    }
    ...
    fun test() {
        ...
        userRoleResourceTestHelper.grantUserClientRole(userUuid = userResponse2.uuid, clientUuid = clientUuid1, userRole = UserRoleModel.CLIENT_ORGANIZATION_CONTENT_MANAGER)
        userRoleResourceTestHelper.grantUserClientRole(userUuid = userResponse2.uuid, clientUuid = clientUuid2, userRole = UserRoleModel.CLIENT_ORGANIZATION_VIEWER)
        userRoleResourceClient.revokeUserOrganizationClientsRoles(userRoleResourceTestHelper.buildUserRoleRevokeOrganizationClientsRequest(
            ...
            assertThat(it.body?.response()?.userUuid).isEqualTo(userResponse2.uuid)
        }
    }
}