...
class AnnotationServiceImpl implements AnnotationService {
    ...
    public Annotation create(final AnnotationCreateDto dto) {
        Assert.notNull(dto, "The AnnotationCreateDto should not be null");
        LOGGER.debug("Creating annotation for the provided dto - {}", dto);
        final User user = userService.getByUuid(dto.getUserUuid());
        final Annotation annotation = new Annotation(
                dto.getProductUuid(), user, dto.getText(), dto.getNumber(), dto.getD1(), dto.getD2(), dto.getD3()
        );
    ...
    public Annotation update(final AnnotationUpdateDto dto) {
        Assert.notNull(dto, "The AnnotationUpdateDto should not be null");
        LOGGER.debug("Updating annotation for the provided dto - {}", dto);
        final Annotation annotation = findByUuid(dto.getUuid())
                .orElseThrow(() -> new EntityNotFoundForUuidException(dto.getUuid(), Annotation.class));
    ...
    public Page<Annotation> findByProductUuid(final AnnotationFindByProductUuidDto dto) {
        Assert.notNull(dto, "The AnnotationFindByProductUuidDto should not be null");
        LOGGER.debug("Retrieving annotation by the provided dto - {}", dto);
        final Page<Annotation> annotations = repository.findByProductUuidAndRemovedIsNull(dto.getProductUuid(), PageRequest.of(dto.getPage(), dto.getSize()));
        LOGGER.debug("Successfully annotation by the provided dto - {}", dto);
        return annotations;
    }
}