...
class TokenInvitationUserFindByInvitationUserUuidServiceUnitTest : AbstractTokenInvitationUserServiceUnitTest() {
    ...
    fun `test not found`() {
        ...
        resetAll()
        expect(tokenInvitationToOrganizationRepository.findByInvitationUserUuid(invitationUserUuid)).andReturn(Optional.empty())
        replayAll()
    ...
    fun test() {
        ...
        val tokenInvitationUser = commonTestHelper.buildTokenInvitationUserToOrganization(invitationOrganizationUser = invitationUser)
        resetAll()
        expect(tokenInvitationToOrganizationRepository.findByInvitationUserUuid(invitationUser.uuid)).andReturn(Optional.of(tokenInvitationUser))
        replayAll()
