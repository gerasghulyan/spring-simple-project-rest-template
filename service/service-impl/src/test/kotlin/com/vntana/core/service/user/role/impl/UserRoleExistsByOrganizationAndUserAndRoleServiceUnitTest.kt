...
class UserRoleExistsByOrganizationAndUserAndRoleServiceUnitTest : AbstractUserRoleServiceUnitTest() {
    ...
    fun `test when noting found`() {
        ...
        expect(userRoleRepository.findAllByOrganization(organizationUuid)).andReturn(listOf())
        replayAll()
    ...
    fun `test admin role`() {
        ...
        val clientRole = commonTestHelper.buildUserClientAdminRole(user = user)
        expect(userRoleRepository.findAllByOrganization(organizationUuid)).andReturn(listOf(ownerRole, adminRole1, clientRole))
        replayAll()
    ...
    fun `test admin role when different users exists in same organization`() {
        ...
        val clientRole = commonTestHelper.buildUserClientAdminRole(user = user)
        expect(userRoleRepository.findAllByOrganization(organizationUuid)).andReturn(listOf(ownerRole, adminRole1, adminRole2, clientRole))
        replayAll()
        val result = userRoleService.existsByOrganizationAndUserAndRole(organizationUuid, user.uuid, role)
        assertThat(result).isTrue()
        verifyAll()
    ...
    fun `test when more then one role found`() {
        ...
        expect(userRoleRepository.findAllByOrganization(organizationUuid)).andReturn(listOf(adminRole1, adminRole2))
        replayAll()
