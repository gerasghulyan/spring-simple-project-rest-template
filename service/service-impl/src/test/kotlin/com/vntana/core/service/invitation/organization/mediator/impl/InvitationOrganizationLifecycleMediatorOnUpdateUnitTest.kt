 ...
 * Time: 6:32 PM
 */
class InvitationOrganizationLifecycleMediatorOnUpdateUnitTest : AbstractInvitationOrganizationUuidAwareLifecycleMediatorUnitTest() {
    ...
    fun test() {
        ...
        expect(applicationEventPublisher.publishEvent(
                InvitationOrganizationUuidAwareLifecyclePayload(invitationOrganization.uuid, EntityLifecycle.UPDATED)
        ))
        replayAll()
        invitationOrganizationUuidAwareLifecycleMediator.onUpdated(invitationOrganization.uuid)
        verifyAll()
    }
}