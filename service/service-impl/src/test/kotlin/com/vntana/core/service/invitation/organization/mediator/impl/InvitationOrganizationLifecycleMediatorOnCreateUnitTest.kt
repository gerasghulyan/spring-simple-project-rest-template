 ...
 * Time: 6:32 PM
 */
class InvitationOrganizationLifecycleMediatorOnCreateUnitTest : AbstractInvitationOrganizationUuidAwareLifecycleMediatorUnitTest() {
    ...
    fun test() {
        ...
        expect(applicationEventPublisher.publishEvent(
                InvitationOrganizationUuidAwareLifecyclePayload(invitationOrganization.uuid, EntityLifecycle.CREATED)
        ))
        replayAll()
        invitationOrganizationUuidAwareLifecycleMediator.onCreated(invitationOrganization.uuid)
        verifyAll()
    }
}