 ...
 * Time: 6:20 PM
 */
class ExistsByUuidUserInvitationToClientServiceIntegrationTest : AbstractInvitationUserToClientServiceIntegrationTest() {

    @Test
    fun `test when not exists`() {
        val invitationUuid = uuid()
        invitationUserToClientService.existsByUuid(invitationUuid).let {
            flushAndClear()
    ...
    fun test() {
        ...
        invitationUserToClientService.existsByUuid(invitation.uuid).let {
            flushAndClear()
